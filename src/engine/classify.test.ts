...
describe('Group A golden oracle (corrected for 2026 rules)', () => {
  ...
  it('Czechia — must beat Mexico AND need South Korea to lose', () => {
    ...
    // Headline should convey that even winning needs South Korea to slip/lose.
    expect(v.headline.toLowerCase()).toMatch(/must win/);
  });

  it('no em-dashes in any generated copy', () => {
    for (const v of verdicts) {
      expect(v.headline).not.toContain('—');
      for (const c of v.conditions) for (const l of c.lines) expect(l).not.toContain('—');
    }
  });
});
